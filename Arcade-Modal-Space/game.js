...
function spawnInvaders(options, answer) {
    ...
    options.forEach((opt, i) => {
        ...
        div.innerText = opt;
        div.style.cssText = `top:-60px; left:${i * spacing + 5}px; font-size:0.7rem; padding:6px 10px; border:2px solid ${colors[i % colors.length]}; box-shadow:0 0 8px ${colors[i % colors.length]}; position:absolute; color:white; border-radius:8px; font-weight:bold; z-index:10;`;
        div.onclick = () => { 
...
function checkAnswer(correct, div) {
    ...
    if (correct) {
        ...
        score += 100; fb.innerText = "EXCELLENT!"; fb.style.color = "#00ff41";
        ...
        score = Math.max(0, score - 50); fb.innerText = "TRY AGAIN!"; fb.style.color = "#ff3131";
...
function update() {
    ...
    invaders.forEach(inv => {
        ...
        if (inv.y > area.clientHeight - 50) endGame(false);
    });
}

function endGame(win) {
    gameActive = false;
    alert(win ? "MISSION ACCOMPLISHED!" : "GAME OVER");
    location.reload();
}
